...
import { Play, Plus, ChevronDown, Star, Clock, Calendar, Film, Tv, Award, User, Check } from "lucide-react";
import { toast } from "sonner";
import { isMovieInWatchlist, toggleWatchlist } from "@/utils/detailsUtils";
...
const Details = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        ...
        setIsAlreadyInList(isMovieInWatchlist(data, mediaType));
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-friends-black">
        <Navbar />
      ...
      <div className="relative">
        ...
        <div className="relative h-[70vh] w-full overflow-hidden">
          <div className="absolute inset-0">
            ...
            <div className="absolute top-0 h-32 w-full friends-gradient-top"></div>
          </div>

          <div className="absolute bottom-0 left-0 p-6 md:p-16 w-full md:w-3/4 z-10 space-y-6">
            <h1 className="friends-title animate-slide-up">
            ...
            <div className="flex flex-wrap gap-3 animate-slide-up" style={{ animationDelay: "0.2s" }}>
              <button
                onClick={handlePlay}
                className="friends-button-primary flex items-center space-x-2 px-8"
              >
                <Play size={18} />
                <span>Play</span>
              </button>

              <button onClick={() => { toggleWatchlist(details, mediaType); setIsAlreadyInList(!isAlreadyInList) }} className="friends-button-secondary flex items-center space-x-2">
                {isAlreadyInList ? <Check size={20} /> : <Plus size={18} />}
                <span>{isAlreadyInList ? "Added to list" : "Add to list"}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 md:px-16 py-8 space-y-8">
            ...
            <div className="space-y-4">
              ...
              {mediaType === "movie" && (
                <div className="flex items-center space-x-2">
                  <Film size={16} className="text-friends-red" />
                  <span>Movie</span>
                </div>
              )}
              ...
              {mediaType === "tv" && (
                <div className="flex items-center space-x-2">
                  <Tv size={16} className="text-friends-red" />
                  <span>TV Series</span>
                </div>
              )}
            </div>
          ...
          {details.credits && details.credits.cast && details.credits.cast.length > 0 && (
            <div className="space-y-4">
              <h2 className="friends-section-title">Cast</h2>
          ...
          {mediaType === "tv" && seasons.length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="friends-section-title">Episodes</h2>
                <div className="relative group">
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-friends-black border border-gray-800 rounded-md shadow-xl z-20 hidden group-hover:block">
                    {seasons.map(season => (
                      <button
                        key={season.id}
                        onClick={() => handleSeasonChange(season.season_number)}
                        className={`w-full text-left px-4 py-2 hover:bg-gray-800 transition ${selectedSeason === season.season_number ? "bg-gray-800" : ""
                          }`}
                      >
                      ...
                      <div className="flex flex-col md:flex-row">
                        <div className="md:w-64 h-40 relative">
                          ...
                          >
                            <div className="w-12 h-12 rounded-full bg-white/90 flex items-center justify-center">
                              <Play size={20} className="text-friends-black ml-1" />
                            </div>
                          </button>
                        </div>
                        ...
                        <div className="p-4 flex-1">
                          <div className="flex justify-between items-start">
                            ...
                            <button
                              onClick={() => handleEpisodePlay(selectedSeason, episode.episode_number)}
                              className="friends-button-primary text-sm hidden md:block"
                            >
                              Play
                            </button>
                          </div>
          ...
          {details.similar && details.similar.results.length > 0 && (
            <div className="mt-8">
              <h2 className="friends-section-title">More Like This</h2>
